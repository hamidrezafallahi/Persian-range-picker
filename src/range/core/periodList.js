...
function PeriodList({ ...props }) {
    const { onChange, setDate, setStep, setZone, componentStep, date, locale = "fa", setCounter, setActiveCompareStep, setCompareDate, showComparison = false, periodClassName, highlightColor = "#f4f4f4", //رنگ برجسته‌کننده برای هاور، نوتیف یا نقاط توجه
    ...
    const filteredPeriod = period.filter((item) => item.step == componentStep ||
    ...
    return (_jsxs(_Fragment, { children: [filteredPeriod.map((item, index) => {
                ...
                return (_jsxs("button", { onClick: () => timeHandler(item), className: `relative flex flex-col items-start w-full h-fit ${periodClassName}`, dir: locale == "fa" ? "rtl" : "ltr", children: [_jsx("div", { style: { color: active ? accentColor : tertiaryColor }, children: item.title }), _jsxs("div", { className: "text-xs", style: {
                                color: active ? neutralColor : tertiaryColor,
                            }, children: [stringDateFrom, componentStep !== ESteps.day && _jsxs(_Fragment, { children: [" - ", stringDateTo] })] }), active && (_jsx("span", { className: ` absolute top-[50%] ${locale == "en" ? "right-5" : "left-5"}`, children: _jsx(TickIcon, { accentColor: accentColor }) })), index < filteredPeriod.length - 1 && (_jsx("div", { style: { backgroundColor: highlightColor }, className: "m-0 w-px h-full" }))] }, index));
            }), showComparison && (_jsx(Comparison, { ...props, switchHandler: switchHandler }))] }));
}
